...
Každá komponenta by měla podporovat zobrazení kontextové nápovědy intuitivním způsobem a to alespoň na úrovni komponenty jako celku. U těch složitějších však lze předpokládat nápovědu u jednotlivých prvků. Nutností je také podpora lokalizace nápovědy. Text nápovědy je každému typu komponenty předán při jejím užití v dokumentu či sestavě.
...
V rámci systému dochází k řadě událostí. Mechanismus upozorňování na ně využívá nastavené komunikační metody (email, Jabber, Matrix) k odeslání upozornění dotčenému uživateli. K upozornění uživatele nedochází, pokud událost, na kterou by měl být upozorněn, sám vyvolal.
...
Některé druhy upozornění se váží na konkrétní případy a uživatel se k jejich odběru musí přihlásit na stránce odpovídajícího případu. Jiný druh je pak vázaný na sadu hraničních hodnot, které jsou definovány politikami.
...
| Byla překročena sledovaná hraniční hodnota **Y**    | {y}   | {y}   | {y}
|===
...
Byla překročena sledovaná hraniční hodnota Y::
Byla překročena některá ze stanovených hraničních hodnot, k jejichž sledování se uživatel přihlásil. Hodnoty, které je možné sledovat, stanovuje programátor politik systému.
