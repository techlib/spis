...
Přehledem se nemíní konkrétní soubor pohledů, ale obecné pojetí představování dat uživatelům tak, aby získali podstatné informace a tedy i přehled. Poskytují vhled do současného, dřívějšího a někdy i možného budoucího stavu věcí. Právě na jejich základě zahajují uživatelé projekty a svými dokumenty stav věcí mění.
