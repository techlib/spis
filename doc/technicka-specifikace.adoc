...
WARNING: Rozepsat se o uspořádání DB jako takovém.

==== Zdroje dat

Relační datové struktury, ze kterých uložené procedury čerpají data pro pohledy, se mohou do systému dostat v zásadě dvěma způsoby. Může jít o data z jiných systémů nebo rozbory místních dokumentů.
...
....
@startuml
title Synchronizace dat z cizího systému\n
...
WARNING: Vlastní dokumenty jsou také zdrojem dat. Využívají se triggery, které data z dokumentů přenáší do tabulek.
