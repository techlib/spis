...
Každá stránka zobrazující uživatelská data (například dokumenty, případy či přehledy) podporuje zobrazení verze pro tisk na bázi stylů pro webový prohlížeč. Ta neobsahuje nepotřebné prvků uživatelského rozhraní a dále přizpůsobuje data tisku.

Systém je možné zkonfigurovat tak, že se použije dodatečný styl, kterým se tisky přizpůsobí grafickému stylu instituce. Dále je možné přiřadit dodatečné styly typům dokumentů a případů.
