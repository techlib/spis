...
Tisk dokumentů je umožněn obdobně jako u tiskových sestav v tématech prostřednictvím běžných prostředků webového prohlížeče. Zvláštní styly pro tisk odstraní ty prvky uživatelského rozhraní, které do dokumentu nepatří a umožní tak plné využití tiskové plochy.

Součástí popisu dokumentu v rámci politiky může být také zvláštní styl pro tisk, který upraví formu dle potřeb dané organizace. Tiskový styl může být nastaven i plošně -- pro všechny dokumenty.

TIP: V první verzi žádné další nástroje pro vytváření tisknutelných dokumentů nebo export do kancelářských formátů nepožadujeme.
...
TIP: V první verzi podporu práce s elektronickými podpisy nepožadujeme. Systém by však měl být připraven na jejich dodatečnou implementaci.
...
+
Pokud byla osoba do role jmenována s příznakem *zastupující*, nemá běžně k dispozici práva, která z role plynou. Může se jich však chopit pomocí patřičného prvku uživatelského rozhraní. Systém uvádí do protokolu informaci o tom, zda-li by danou akci mohla osoba provést aniž by se chopila práva zástupce.
